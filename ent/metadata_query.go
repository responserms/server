...
func (mq *MetadataQuery) QueryMapType() *MapTypeQuery {
	query := &MapTypeQuery{config: mq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := mq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (mq *MetadataQuery) sqlAll(ctx context.Context) ([]*Metadata, error) {
	...
	if query := mq.withUser; query != nil {
		...
		for i := range nodes {
			if fk := nodes[i].user_metadata; fk != nil {
				ids = append(ids, *fk)
	...
	if query := mq.withMapType; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Metadata)
		for i := range nodes {
			if fk := nodes[i].map_type_metadata; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "map_type_metadata" returned %v`, n.ID)
			}
			for i := range nodes {
...
func (mq *MetadataQuery) sqlQuery() *sql.Selector {
...
func (ms *MetadataSelect) Scan(ctx context.Context, v interface{}) error {
	...
	ms.sql = ms.MetadataQuery.sqlQuery()
