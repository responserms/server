...
func (pq *PlayerQuery) QueryServers() *ServerQuery {
	query := &ServerQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PlayerQuery) sqlAll(ctx context.Context) ([]*Player, error) {
	...
	if pq.withMetadata != nil || pq.withUser != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := pq.withMetadata; query != nil {
		...
		for i := range nodes {
			if fk := nodes[i].player_metadata; fk != nil {
				ids = append(ids, *fk)
	...
	if query := pq.withUser; query != nil {
		...
		nodeids := make(map[int][]*Player)
		for i := range nodes {
			if fk := nodes[i].player_user; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "player_user" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := pq.withIdentifiers; query != nil {
		...
		for _, n := range neighbors {
			fk := n.player_identifier_player
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "player_identifier_player" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "player_identifier_player" returned %v for node %v`, *fk, n.ID)
			}
...
func (pq *PlayerQuery) sqlQuery() *sql.Selector {
...
func (ps *PlayerSelect) Scan(ctx context.Context, v interface{}) error {
	...
	ps.sql = ps.PlayerQuery.sqlQuery()
