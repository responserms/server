...
type PlayerIdentifier struct {
	...
	player_identifier_metadata *int
	player_identifier_player   *int
}
...
func (*PlayerIdentifier) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		...
		case playeridentifier.FieldType, playeridentifier.FieldIdentifier:
			values[i] = &sql.NullString{}
		case playeridentifier.FieldCreatedAt, playeridentifier.FieldUpdatedAt:
			values[i] = &sql.NullTime{}
		case playeridentifier.ForeignKeys[0]: // player_identifier_metadata
			values[i] = &sql.NullInt64{}
		case playeridentifier.ForeignKeys[1]: // player_identifier_player
			values[i] = &sql.NullInt64{}
		default:
	...
	for i := range columns {
		...
		case playeridentifier.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field player_identifier_metadata", value)
			} else if value.Valid {
				pi.player_identifier_metadata = new(int)
				*pi.player_identifier_metadata = int(value.Int64)
			}
		case playeridentifier.ForeignKeys[1]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field player_identifier_player", value)
			} else if value.Valid {
				pi.player_identifier_player = new(int)
				*pi.player_identifier_player = int(value.Int64)
			}
		}
	}
	return nil
...
func (pi *PlayerIdentifier) QueryMetadata() *MetadataQuery {
	return (&PlayerIdentifierClient{config: pi.config}).QueryMetadata(pi)
}
...
func (pi *PlayerIdentifier) String() string {
	...
	builder.WriteString(pi.Type)
	builder.WriteString(", identifier=")
	builder.WriteString(pi.Identifier)
	builder.WriteByte(')')
