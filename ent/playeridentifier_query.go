...
func (piq *PlayerIdentifierQuery) QueryMetadata() *MetadataQuery {
	query := &MetadataQuery{config: piq.config}
...
func (piq *PlayerIdentifierQuery) Clone() *PlayerIdentifierQuery {
	...
	return &PlayerIdentifierQuery{
		...
		withMetadata: piq.withMetadata.Clone(),
...
func (piq *PlayerIdentifierQuery) sqlAll(ctx context.Context) ([]*PlayerIdentifier, error) {
	var (
		...
		_spec       = piq.querySpec()
		loadedTypes = [2]bool{
			piq.withMetadata != nil,
			piq.withPlayer != nil,
		}
	)
	if piq.withMetadata != nil || piq.withPlayer != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := piq.withMetadata; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*PlayerIdentifier)
		for i := range nodes {
			if fk := nodes[i].player_identifier_metadata; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "player_identifier_metadata" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := piq.withPlayer; query != nil {
		...
		nodeids := make(map[int][]*PlayerIdentifier)
		for i := range nodes {
			if fk := nodes[i].player_identifier_player; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "player_identifier_player" returned %v`, n.ID)
			}
			for i := range nodes {
...
func (piq *PlayerIdentifierQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if ps := piq.order; len(ps) > 0 {
		_spec.Order = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector, playeridentifier.ValidColumn)
			}
		}
	}
	return _spec
...
func (piq *PlayerIdentifierQuery) sqlQuery() *sql.Selector {
	builder := sql.Dialect(piq.driver.Dialect())
	t1 := builder.Table(playeridentifier.Table)
	selector := builder.Select(t1.Columns(playeridentifier.Columns...)...).From(t1)
	if piq.sql != nil {
		selector = piq.sql
		selector.Select(selector.Columns(playeridentifier.Columns...)...)
	}
	for _, p := range piq.predicates {
		p(selector)
	}
	for _, p := range piq.order {
		p(selector, playeridentifier.ValidColumn)
	}
	if offset := piq.offset; offset != nil {
...
func (pigb *PlayerIdentifierGroupBy) sqlQuery() *sql.Selector {
	...
	for _, fn := range pigb.fns {
		columns = append(columns, fn(selector, playeridentifier.ValidColumn))
	}
...
func (pis *PlayerIdentifierSelect) Scan(ctx context.Context, v interface{}) error {
	...
	pis.sql = pis.PlayerIdentifierQuery.sqlQuery()
...
func (pis *PlayerIdentifierSelect) sqlQuery() sql.Querier {
	selector := pis.sql
