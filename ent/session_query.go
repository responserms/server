...
func (sq *SessionQuery) QueryToken() *TokenQuery {
	query := &TokenQuery{config: sq.config}
...
func (sq *SessionQuery) sqlAll(ctx context.Context) ([]*Session, error) {
	...
	if sq.withToken != nil || sq.withUser != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := sq.withToken; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Session)
		for i := range nodes {
			if fk := nodes[i].token_session; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "token_session" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Token = n
			}
		}
	}
